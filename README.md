...
It deploys to [unfollow.garden](https://unfollow.garden) on Cloudflare Pages as
a static bundle — Pages serves files, and there is still nothing in the request
path that could see a decision.
...
| Hosting         | Cloudflare Pages, static bundle only — no Worker, no origin server               |
...
- **Redirect `www` to the apex.** `client_id` is the URL the metadata is served
  ...
  everyone else.
- **Ship `static/_headers`.** It carries `frame-ancestors` and the privacy
  headers. The rest of the Content-Security-Policy is generated by SvelteKit
  into a `<meta>` tag, because it has to hash the framework's own inline
  bootstrap script — see `CONTEXT.md` before changing either half.
